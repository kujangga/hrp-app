...
import { Package, ShoppingCart, ArrowRight, Filter, DollarSign, CheckCircle2, X, Minus, Plus } from 'lucide-react'
...
export default function SelectEquipment() {
    ...
    const selectedEquipment = getItemsByType('equipment')

    // Mock data for equipment
    const equipment = [
        {
            id: 'eq1',
            name: 'Canon EOS R5 + 24-70mm',
            category: 'Camera',
            dailyRate: 800000,
            description: 'Professional full-frame mirrorless camera with versatile zoom lens.',
            features: ['45MP Sensor', '8K Video', 'Image Stabilization', 'Dual Card Slots'],
        },
        {
            id: 'eq2',
            name: 'Sony A7 IV + 85mm f/1.4',
            category: 'Camera',
            dailyRate: 750000,
            description: 'High-resolution camera perfect for portraits and events.',
            features: ['33MP Sensor', '4K 60fps', 'Fast Autofocus', 'Weather Sealed'],
        },
        {
            id: 'eq3',
            ...
            category: 'Lighting',
            dailyRate: 200000,
            description: 'Powerful LED continuous lighting for video and photography.',
            features: ['60W Power', 'Daylight Balanced', 'Silent Operation', 'Remote Control'],
        },
        {
            id: 'eq4',
            ...
            category: 'Support',
            dailyRate: 150000,
            description: 'Lightweight and stable carbon fiber tripod system.',
            features: ['Carbon Fiber', 'Fluid Head', '10kg Capacity', 'Quick Release'],
        },
        {
            id: 'eq5',
            ...
            category: 'Stabilizer',
            dailyRate: 600000,
            description: 'Professional gimbal stabilizer for smooth video shots.',
            features: ['4.5kg Payload', 'Automated Modes', 'Long Battery Life', 'Briefcase Handle'],
        },
        {
            id: 'eq6',
            ...
            category: 'Audio',
            dailyRate: 250000,
            description: 'Compact wireless microphone system for clear audio recording.',
            features: ['2 Transmitters', '200m Range', 'On-Board Recording', 'Universal Compatibility'],
        }
    ]

    const categories = ['All', 'Camera', 'Lighting', 'Support', 'Stabilizer', 'Audio']
    ...
    const handleQuantityChange = (item: typeof equipment[0], delta: number) => {
        const currentQty = getQuantity(item.id)
        const newQty = currentQty + delta

        if (newQty <= 0) {
            if (currentQty > 0) {
                removeItem(item.id, 'equipment')
            }
            ...
            updateItemQuantity(item.id, 'equipment', newQty)
        }
    }

    const handleContinue = () => {
        const nextStep = getNextStep(pathname || '/booking/equipment')
        if (nextStep) {
                    ...
                    <div className="mx-auto max-w-7xl">
                        <motion.div className="mb-8 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                            ...
                            <p className="text-lg text-white/70">
                                Browse and rent professional photography equipment
                            </p>
                            ...
                            <motion.div className="lg:col-span-1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
                                <div className="rounded-2xl border border-white/10 bg-white/95 p-6 shadow-xl backdrop-blur-xl">
                                    ...
                                    <div className="space-y-6">
                                        <div>
                                            <label htmlFor="category" className="mb-2 flex items-center gap-2 text-sm font-semibold" style={{ color: COLORS.NAVY_DARK }}>
                                                <Package className="h-4 w-4" />
                                                Category
                                            </label>
                                            <select id="category" name="category" value={filters.category} onChange={handleFilterChange} className="block w-full rounded-xl border-2 border-gray-200 bg-white px-3 py-3 text-sm transition-all duration-200 focus:border-[#7D97B6] focus:outline-none focus:ring-4 focus:ring-[#7D97B6]/20" style={{ color: COLORS.NAVY_DARK }}>
                                                {categories.map(cat => (
                                ...
                                <motion.div className="mt-8 overflow-hidden rounded-2xl border border-white/10 bg-white/95 shadow-xl backdrop-blur-xl" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }}>
                                    <div className="p-6">
                                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                            <div>
                                                <h3 className="mb-1 text-xl font-bold" style={{ color: COLORS.NAVY_DARK }}>Selected Equipment ({selectedEquipment.length} items)</h3>
                                                <p className="text-sm" style={{ color: COLORS.SLATE_MEDIUM }}>
                                                    {selectedEquipment.length > 0 ? 'Continue to next step or add more equipment' : 'Select equipment to continue or skip this step'}
                                                </p>
                                            </div>
                                            <div className="flex gap-3">
                                                ...
                                                <motion.button onClick={handleContinue} disabled={selectedEquipment.length === 0} className={`flex items-center gap-2 rounded-xl px-6 py-3 font-semibold text-white shadow-lg transition-all duration-200 ${selectedEquipment.length === 0 ? 'cursor-not-allowed bg-gray-400' : 'bg-[#7D97B6] hover:bg-[#6a8399] hover:shadow-xl'}`} whileHover={selectedEquipment.length > 0 ? { scale: 1.02 } : {}} whileTap={selectedEquipment.length > 0 ? { scale: 0.98 } : {}}>
                                                    Next Step
